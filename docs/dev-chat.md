...
- Run its browser smoke test and initialize the DEV profile. Complete MCP setup only when testing
  ...
  tunnel. Create the ChatGPT connector as `Native2 DEV`; keep `Native2` unchanged.
...
the launcher-owned DEV tunnel ready so ChatGPT can create and validate the configured DEV connector
(`Native2 DEV` by default, or `<connector-name> DEV` when `--connector-name` is used) before a
