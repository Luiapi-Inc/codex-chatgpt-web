...
function locateCodexInterruptHook(text: string, installed: InstalledCodexInterruptHook): Array<{
  ...
  const managedStateHeader = `[hooks.state.${JSON.stringify(installed.stateKey)}]`;
  const managedStateOffset = installed.fragment.indexOf(managedStateHeader);
  if (managedStateOffset < 0) throw new Error("Codex interrupt lifecycle hook journal fragment is invalid");
  const ownedHookPrefix = installed.fragment.slice(0, managedStateOffset);
  const ownedState = installed.fragment.slice(managedStateOffset, marker);
  // Native config writes normalize CRLF to LF; commands and owned fields must still match exactly.
  const hookPattern = new RegExp(hookTextPattern(ownedHookPrefix), "g");
  const hookMatch = hookPattern.exec(text);
  if (!hookMatch || hookPattern.exec(text)) {
    throw new Error("Codex interrupt lifecycle hook changed after setup; refusing to overwrite it");
  }
  const statePattern = new RegExp(hookTextPattern(ownedState), "g");
  const stateMatch = statePattern.exec(text);
  if (!stateMatch || statePattern.exec(text)) {
    throw new Error("Codex interrupt lifecycle hook changed after setup; refusing to overwrite it");
  }
  const first = hookMatch.index;
  const hookEnd = first + hookMatch[0].length;
  const stateStart = stateMatch.index;
  const stateEnd = stateStart + stateMatch[0].length;
  if (stateStart < hookEnd) {
    throw new Error("Codex interrupt lifecycle hook changed after setup; refusing to overwrite it");
  }
  ...
  if (managedMarkerCount(text) !== 1 || endMarker < 0
    || (endMarker >= first && endMarker < hookEnd)
  ...
  const interleavedConfig = text.slice(hookEnd, stateStart);
  const appendedConfig = text.slice(stateEnd, endMarker < first ? undefined : endMarker);
  const insertedConfig = `${interleavedConfig}${appendedConfig}`;
  const firstAssignment = insertedConfig.split(/\r\n|\n|\r/)
    .map(line => line.trim()).find(line => line && !line.startsWith("#"));
  if (firstAssignment && !/^\[\[?.+\]\]?(?:\s*#.*)?$/.test(firstAssignment)) {
    throw new Error("Codex interrupt lifecycle hook changed after setup; refusing to overwrite it");
  }
  if (insertedConfig.includes("[[hooks.Interrupt.hooks]]")) {
    throw new Error("Codex interrupt lifecycle hook changed after setup; refusing to overwrite it");
  }
  if (insertedConfig.includes(`[hooks.state.${JSON.stringify(installed.stateKey)}.`)) {
    throw new Error("Codex interrupt lifecycle hook changed after setup; refusing to overwrite it");
  }
  if (text.includes(`[hooks.state.${JSON.stringify(installed.stateKey)}.`)) {
    throw new Error("Codex interrupt lifecycle hook changed after setup; refusing to overwrite it");
  }
  if (text.slice(stateEnd).includes("[[hooks.Interrupt.hooks]]")) {
    throw new Error("Codex interrupt lifecycle hook changed after setup; refusing to overwrite it");
  }
  ...
  return [{ start: first, end: hookEnd }, { start: stateStart, end: stateEnd }, { start: endMarker, end: end + trailingLength }];
}
