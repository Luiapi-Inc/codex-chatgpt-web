...
const en = {
  ...
  devConnectorIsolationNotice: "Create a separate connector with the exact DEV name below. Keep Native2 unchanged so production and repository development can run side by side.",
  ...
  connectorMigrationNotice: "Upgrading from Codex Native2 or Codex Native? Leave the old connector untouched and create Native2 as a new connector. Do not rename or refresh the old connector. Verify runtime rejects a legacy-only setup.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  devConnectorIsolationNotice: "请使用下方准确的 DEV 名称新建独立连接器。保留 Native2 不变，让生产使用和仓库开发可以同时运行。",
  ...
  connectorMigrationNotice: "从 Codex Native2 或 Codex Native 升级？请保留旧连接器不变，并新建名为 Native2 的连接器。不要重命名或刷新旧连接器。仅有旧连接器时，运行时验证会明确失败。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  devConnectorIsolationNotice: "下記の正確な DEV 名で個別のコネクタを作成してください。Native2 は変更せず、本番利用とリポジトリ開発を並行して実行できる状態を保ちます。",
  ...
  connectorMigrationNotice: "Codex Native2 または Codex Native からアップグレードする場合は、古いコネクタを変更せず、新しい Native2 コネクタを作成してください。古いコネクタの名前変更や更新は行わないでください。古いコネクタしかない場合、ランタイム検証は明示的に失敗します。",
...
const ko: Record<keyof typeof en, string> = {
  ...
  devConnectorIsolationNotice: "아래의 정확한 DEV 이름으로 별도의 커넥터를 만드세요. Native2는 변경하지 않아 프로덕션 환경과 저장소 개발 환경을 동시에 사용할 수 있도록 유지하세요.",
  ...
  connectorMigrationNotice: "Codex Native2 또는 Codex Native에서 업그레이드하는 경우 기존 커넥터는 그대로 두고 Native2라는 새 커넥터를 만드세요. 기존 커넥터의 이름을 변경하거나 새로고침하지 마세요. 기존 커넥터만 있는 구성은 런타임 검증에서 거부됩니다.",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  devConnectorIsolationNotice: "請使用下方正確的 DEV 名稱新增獨立連接器。保留 Native2 不變，讓正式環境與儲存庫開發可以同時執行。",
  ...
  connectorMigrationNotice: "從 Codex Native2 或 Codex Native 升級？請保留舊連接器不變，並新增名為 Native2 的連接器。不要重新命名或重新整理舊連接器。僅有舊連接器時，執行階段驗證會明確失敗。",
