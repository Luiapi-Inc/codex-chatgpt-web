...
test("runtime message localization preserves other languages and unknown backend messages", () => {
  const { copyFor, localizeRuntimeMessage } = loadI18nModule();
  const connectorNames = ["Native2", "Native $&", "Native $'", "Native $`", 'Native "quoted"', "Native \\path"];
  ...
  assert.equal(
    localizeRuntimeMessage(copyFor("ja"), 'ChatGPT connector "Native2" is available', "wrong-id", "ja"),
    'ChatGPT connector "Native2" is available',
  );
  assert.equal(
  ...
  assert.equal(
    localizeRuntimeMessage(copyFor("ja"), 'ChatGPT connector "Native2" is available (warning)', "connector", "ja"),
    'ChatGPT connector "Native2" is available (warning)',
  );
});
